...
# 	if(filters.get('customer_sub')):conditions += f" AND tsi.customer_sub='{filters.get('customer_sub')}' "
...
#
# 	#SQL Query
# 	data = frappe.db.sql(f""" SELECT tsi.name, tsi.posting_date, tsi.customer, tst.sales_person, tsi.customer_sub, tsi.grand_total
# 								FROM `tabSales Invoice` tsi
# 								Inner Join `tabSales Team` tst on tsi.name = tst.parent
# 								inner Join tabCustomer tc on tsi.customer = tc.name
# 								WHERE tsi.docstatus = 1 AND (tsi.posting_date BETWEEN '{_from}' AND '{to}')
# 							 	{conditions} """)
# 	return data
#
# def get_columns():
# 	return [
# 	   "Sales Invoice: Data/Sales Invoice:200",
# 	   "Posting Date: Date/ Posting Date:120",
# 	   "Customer Name: Data/Customer:200",
# 	   "Sales Person: Data/Sales Person:200",
# 	   "Customer Sub: Data/Customer Sub:200",
	...
	def get_invoice_details(self):
		...
		if self.party_type == "Customer":
			si_list = frappe.db.sql(
				"""
				select name, due_date, po_no, customer_sub
		...
		journal_entries = frappe.db.sql(
			"""
			select name, due_date, bill_no, bill_date, customer_sub
